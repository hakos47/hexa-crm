...
describe("browser store key rename + legacy read", () => {
  ...
  it("uses hexa-crm-store-v5 as the canonical localStorage key", () => {
    expect(__browserStoreKeyForTests()).toBe("hexa-crm-store-v5");
    expect(__legacyBrowserStoreKeysForTests()).toContain("nix-c-store-v5");
  });
